...
const LoginPage = () => {
    ...
    const display = async () => {
      try {
        ...
        if (error.response && error.response.status === 403) {
          setLoginError("Your account has been banned by the admin.");
        } else {
          setLoginError("An error occurred while generating OTP. Please try again.");
        }
      }
    };
  ...
  return (
    <div className="relative w-full h-full bg-cover bg-center" style={{ backgroundImage: `url(${loginBg})` }}>
      <div className="absolute top-0 left-0 w-full h-full bg-black opacity-50"></div>
      <div className="relative z-10 flex items-center justify-center w-full h-full px-4 sm:px-8">
        <div className="w-full max-w-sm px-8 py-8 bg-gray-950 bg-opacity-60 rounded-lg">
          ...
          <form onSubmit={display} className="space-y-4">
            ...
            <div className="text-sm text-center mt-4">
              <a href="#" className="text-blue-500 hover:underline" onClick={() => navigate("/sendemail")}>
                Forgot password?
              </a>
            </div>
